ラブリーコンポーザ 操作説明書 (ver.1.2.0 + α) 
##############################################################################

この説明書はLovelyComposerのホーム画面やF1キーから開くことができます。内容は最新版プログラムとは異なる場合があります。

さくいん
=================================================================

* :ref:`id_music_editor_jp`
* :ref:`id_export_screen_jp`
* :ref:`id_shortcut_keys_jp`
* :ref:`id_q_and_a_jp`
...
曲データを音声ファイルやMIDIファイル、その他の形式で出力するための画面です。
...
* **MUSIC - 1 / ALL ボタン** … 全曲をファイル出力するか、選択した1曲のみ出力するかを選びます。1 MUSICを選択した場合は、右の曲番号セレクタで対象の曲を選べるようになります。（初期値は作曲画面で選んでいた曲の番号となります。）
* **TYPE** … どの種類のファイルとして出力するかを選びます。種類によって画面下部のオプション設定項目が変わります。
* **FOLDER** … エクスポート先のフォルダパスを指定します。左側のフォルダボタンを押すとフォルダ選択ダイアログが開きます。
* **フォルダオープンアイコン** … エクスポート先フォルダをOS側のエクスプローラ等で開きます。
* **AUTOボタン** … 有効時（カラー表示の場合）、エクスポート処理完了時に自動的にエクスポート先フォルダを開きます。
* **ON SAVEボタン** … 有効時、エディット画面で手動セーブ操作をするとセーブ完了後に自動でエクスポートを行います。
* **EXPORTボタン** … 現在の設定でファイル出力を実行します。
...
Lovely Composer形式のjsonlデータで、かつリズムパターンの音を音符データとして展開したものを出力します。
...
外部拡張アドオンを呼び出して変換します。アドオンの設置場所は **/ユーザードキュメントフォルダ/LovelyComposer/addon/export/** となります。アドオンは :ref:`自作することもできます。<id_addons_jp>` 

* **ADDON** … 左側のリストボックスでアドオンを選択します。右側のリストボックスでアドオンごとに用意された変換オプションを選択します。
* **INPUT** … アドオンの入力ファイルに使われるファイル種類を選択します。アドオン側で入力ファイルが指定されている場合はユーザは選択不可になります。入力ファイルはアドオン実行の直前に現在のエクスポート設定で生成されます。
* **FILENAME** … 出力ファイル名を選択します。 **F.NAME** は曲データに設定されたファイル名（設定されてない場合は曲番号）、 **No_F.N** は曲番号とファイル名をアンダーバーでつないだもの、 **No.** は曲番号のみになります。
* **COMMAND** … 最終的に実行されるコマンドが表示されます。コマンドは直接書き換えることもできます。入力ファイル名等の :ref:`変数<id_export_addons_command_vars_jp>` が利用できます。
...
* 選択したものを削除 … Delete
* フルスクリーン化 … Alt + Enter
* 現在画面の説明書をブラウザ表示 … F1 　(ver.1.2.6以降)
* 公式ガイドをブラウザ表示 … F2 　(ver.1.2.6以降)
...
.. _id_gamepad_button_layouts_jp:

ゲームコントローラー ボタン配置 (ver.1.3.2以降)
##############################################################

* **十字キー/左アナログスティック** ...... カーソルを移動します。 デフォルトの速度はシステム設定で変更できます。
* **A** ...... 左クリック
* **B** ...... 右クリック
* **X** ...... シフトキー（※ 他のボタンと組み合わせて使用します）
* **Y** ...... 曲の再生/停止
* **L1/R1** ...... ページ移動
* **L2/R2** ...... 押している間のカーソル速度を変更します。 ※音程を正確に入力したい場合は、R2を押しながら入力すると入力を間違えにくくなります。
* **X + Y** ...... ページループのオン/オフ
* **X + (L / R)** ...... アンドゥ/リドゥ
* **X+(左/右)** ...... パーツ選択
* **X+(上/下)** ...... ツール選択
* **SELECT + (L/R)** ...... 曲の選択
* **SELECT + (X / Y)** ...... 編集モード（ノート/ボリューム/パン）を選択
* **SELECT + START + L + R** ...... アプリを終了する
* **SELECT + START + X + Y** ...... すべてのユーザー設定を初期化します

ABXYボタンの配置は設定で左右切り替え可能です。

コントローラーの設定を誤って何も操作できなくなった場合には、マウスをつないで設定し直すか、設定を初期化してください。
...
エクスポートアドオン (ver.1.3.3以降)
=================================================

.. image:: ../img/export_mode_addon_option.png

ユーザ側で用意した変換プログラム等を、エクスポート時に呼び出して自動実行できるしくみが用意されています。
コマンドラインで実行ファイルやスクリプトに引数を渡して実行します。
ゲーム制作等に活用でき、少しの作業で自作することもできます。


フォルダ構造
------------------------------------------------------

アドオンは専用の設定ファイルと実行ファイルやスクリプトを1つのフォルダにまとめたものです。
アドオンの設置場所は **/ユーザードキュメントフォルダ/LovelyComposer/addon/export/** となります。
...
以下のサンプルのような設定テキストファイルを **config.json** という名前でアドオンフォルダに1つ用意する必要があります。

.. image:: ../img/export_addon_config_example.png

* **addon_name** ... アドオン名を表す任意の文字列です。（ver.1.3.3時点ではアプリ上で表示されません）
* **version** ... バージョンを表す任意の文字列でです。（ver.1.3.3時点ではアプリ上で表示されません）
* **help** ... 説明文に使われる任意の文字列です。画面下などに表示されます。
* **bubbles_help** ... 吹き出し型ポップアップヘルプに使われる任意の文字列です。
* **options** ... 1つから複数のオプション選択項目をオブジェクトの配列で設定します。オプションが1つの場合でも配列である必要があります。

各オプションについて
-------------------------------------

* **name** ... オプション名の任意の文字列です。
* **command** ... 実行するコマンドラインを指定します。変数が使用できます（後述）。※ 拡張子を除いた実行ファイル名を指定するとWindowsの場合は.exeを追加したファイルが呼ばれ、Linuxの場合は拡張子なしのファイルが呼ばれます
* **input_type** ... 入力ファイルの形式を選択します。 **audio / midi / raw / origin** のいずれかを文字列で指定します。 **origin** はラブリーコンポーザの曲データ形式そのもので、 **raw** はそれに加えてリズムパターンを音符に展開して追加したものです。エクスポート画面で選択された設定で生成し、それを入力ファイルとして使用します。
* **input_audio_format** ... **input_type** で **audio** を指定した場合のみ有効な項目で、 **wav / flac / mp3 / ogg / opus** のいずれかを文字列で指定します。必須ではありません。
* **output_extension** ... 出力ファイルの拡張子を文字列で指定します。ピリオドは不要です。

コマンド中のファイルパスは最終的に引用符で囲まれる必要があります。（変数{i}と{o}に代入される文字列には最初から引用符が含まれるので囲む必要はありません。）


.. _id_export_addons_command_vars_jp:

コマンド変数
-------------------------------------

**command** には以下の変数が使用できます。

* **{i}** ... 入力ファイル名が代入されます。引用符で囲まれたフルパス文字列です。
* **{o}** ... 出力ファイル名が代入されます。引用符で囲まれたフルパス文字列です。入力ファイル名の拡張子をオプションの **output_extension** で指定したものに置き換えたものになります。 
* **{basename}** ... 拡張子を除いたファイル名のみが代入されます。
* **{addondir}** ... 選択されたアドオンまでのフォルダパスがフルパスで代入されます。
* **{exportdir}** ... エクスポート先のフォルダパスがフルパスで代入されます。

注意点
---------------------------

* 文字列についてはアプリケーション上では半角英数と一部記号しか表示されませんのでご注意ください。
* アドオンはLovely Composer起動時に読み込まれます。アドオンの内容を更新して再読み込みしたい場合は再起動してください。
* ファイルパスは最終的に引用符で囲まないと正常に動作しません。 **basename** や **addondir** などを使う場合は引用符で囲んでご使用ください。
* エラー内容の詳細はログファイルに出力されます。 **/プログラムフォルダ/log/app_log.log** をテキストエディタで開いて参照してください。ログはアプリ終了時にテキストに書き出されます。
* 同梱されているサンプルアドオンの内容も参考にしてください。
* 同梱の各アドオンについてはサポートを一切行いません（いかなる不利益の責任も改善の義務も負いません）。また各アドオンの開発者もいかなるサポートの義務や不利益の責任も一切負いません。