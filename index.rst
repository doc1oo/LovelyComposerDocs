...
再生ボタン（大きな三角）を押すと曲を再生し、停止ボタン（四角）を押すと再生を停止します。
「1ページリピート」ボタン（円形の矢印）を押し、緑色にすると、現在開いているページのみでループ再生されるようになります。（この設定は曲ごとには保存されない、一時的なものです。）

早送りボタンを押すと、次のページに移動します。
巻き戻しボタンを押すと、前のページに移動します。

ループ開始位置ボタンを押し緑色にすると、ループ再生の開始位置をそのページにセットします。その状態でもう一度押すと、灰色になり開始位置指定が解除されます。
ループ終了位置ボタンについても同様です。

一番右のテンポ設定スライダーでは、曲の再生速度が設定できます。数値が小さい（カーソルが左に近づく）ほど再生速度が速くなります。

  再生停止中に停止ボタンを押すと、曲の先頭に戻ります。
...
* コピー
* ペースト
* 選択範囲の対象レイヤー指定（選択中のパートのみ / 全パート対象）
* 選択範囲の透過コピー指定（通常コピー / 透過コピー）
...
定番のコード進行パターンを一覧から選んで入力できるツールです。コードの知識がなくても、実際に音を鳴らして聞きながら選べます。
