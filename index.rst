...
作曲時に一番メインとなる編集画面で、ここで音符等を入力・編集することで曲を作っていきます。

音楽の五線譜と同様に、縦軸は音程、小節が縦線で区切られています。より五線譜風の表示にも変更可能です。

補助的な情報表示、例えばループ位置やミュート状態等の表示も行われます。
...
Shiftキーを押している間は、カーソルを水平移動に固定できます。
  ...
  プロモードの音量編集では、音量を指定できます。パン編集ではパンを指定できます。またマウスホイールの上下で現在選択中のパンを変更できます。

消しゴムツール
-----------------------------

左クリック、ドラッグで音符の削除ができます。
...
五線譜風ピアノロール表示は以下のようになります。（あくまでもピアノロールベースなので、線が等間隔でなかったりします。）
...
* 停止ボタン（四角）を押すと再生を停止します。再生停止中に停止ボタンを押すと、曲の先頭に戻ります。
